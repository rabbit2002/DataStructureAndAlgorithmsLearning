...
public class L105TimeOut {
    public static void main(String[] args) {
        ...
        int[] problemScore = new int[numberProblem];
        int[] problemAnswer = new int[numberProblem];

        for (int i = 0; i < numberProblem; i++) {
            problemScore[i] = scanner.nextInt();
        }
        ...
        for (int i = 0; i < numberStudent; i++) {
            ...
            for (int j = 0; j < numberProblem; j++) {
                if (problemAnswer[j] == scanner.nextInt()) {
                    studentScore += problemScore[j];
                }
            }
